 ...
 * 记账下所有会话的明文 JSONL 打包成一个 ZIP 流式下发。打包走 fflate 的
 ...
 * 持有一个会话的 artifact 文本。
 *
 * 为什么用 webServer 路由而不是 RPC：工作区可能包含几十个会话、合计
 * 上百 MB 明文，走 JSON 信封 base64 编码会膨胀 33% 且整体驻留内存；HTTP
 * GET + 流式 ZIP 与官方 `/api/session.export` 同款，浏览器端一个
 * `<a download>` 即可触发。
 */
